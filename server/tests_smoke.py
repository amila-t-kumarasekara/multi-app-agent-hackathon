...
o.handle_email(email); assert crm.contacts == {} and all(a["status"] == "DRY_RUN" for a in db.actions_for(db._x("SELECT id FROM runs").fetchone()["id"]) if a["tool"] != "search_crm_contact")
print("7 dry run            OK")
