...
def build(case):
    ...
    db = DB(os.path.join(tempfile.mkdtemp(), "e.db"))
...
def main():
    ...
    for path in cases:
        ...
        db, ex, crm, gmail, slack, cal = build(case)
        ...
        rows.append((case["id"], case.get("bucket", "-"), "PASS" if ok else "FAIL", "; ".join(fails)))
    w = max(len(r[0]) for r in rows)
    print(f"\n{'case':<{w}}  {'bucket':<10} result  detail")
    for r in rows: print(f"{r[0]:<{w}}  {r[1]:<10} {r[2]:<6}  {r[3]}")
